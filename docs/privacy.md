...
* Are transmitted securely to our backend service for identification.
* Are processed to extract card attributes such as player name, year, set, card number, parallel, or grade.
* Are processed in memory for identification and are not retained after processing is complete.
...
We use Microsoft Azure Application Insights to monitor system reliability and performance. Diagnostic telemetry generated by our hosting provider may include technical identifiers such as IP address for security and operational monitoring. We do not use this information for advertising, profiling, or cross-site tracking.
...
The Extension only processes content necessary to capture the user-selected image region during a user-initiated scan.
...
Subscription payments are processed by our payment provider. We do not store full payment card details.

### 5.2 CardHedge

To provide comps and historical sales data, we send structured card identification details (such as player, year, set, card number, parallel, and grade if applicable) to CardHedge and receive market data in response. No personal account or payment information is shared with CardHedge.
...
* Captured images are processed in memory and are not permanently stored.  
